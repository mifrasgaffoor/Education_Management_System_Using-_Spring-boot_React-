...
const EditClassRoom = () => {
  ...
  useEffect(() => {
    ClsRoomService.getClsByid(id)
      .then((res) => {
        setnameOfclass(res.data.nameOfclass);
  ...
  const updateCls = (e) => {
    e.preventDefault();
    const cls = { id, nameOfclass, classDesc };
    if (id) {
      ClsRoomService.updateCls(cls)
        .then((res) => {
  ...
  return (
    <div className="container_teacher">
      <h2 className="text-center">Update ClassRoom </h2>
      <div className="row">
